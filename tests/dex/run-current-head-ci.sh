...
write_receipt() {
  ...
  diagnostic=${2:-none}
  {
    ...
    for stage in compiler_checkout compiler_build compiler_api_install backend_build wegert_direct_dex native_activity_shells reddit_direct_dex dex_generation dex_validation art_execution; do
      case " $passed " in
...
run_stage make -C "$repo_root" preflight IDRIC="$compiler" IDRIC_REPO="$idric_repo" IDRIC_COMPILER_REF="$compiler_ref"
...
run_stage env IDRIC="$compiler" bash "$repo_root/tests/dex/reddit-host-acceptance.sh"
passed="$passed reddit_direct_dex"
