...
[ -n "$qemu_arm_static" ] || {
    printf '%s\n' 'FAIL: qemu-arm-static or qemu-armhf-static is required for Debian armhf bootstrap' >&2
    exit 1
}
...
sudo debootstrap \
    ...
    --arch=armhf \
    --variant=minbase \
    --include=linux-image-armmp,kmod,busybox \
...
if ! sudo chroot "$rootfs" /debootstrap/debootstrap --second-stage; then
    printf '%s\n' 'FAIL: Debian armhf second-stage bootstrap did not execute; qemu-arm binfmt must be active' >&2
    exit 1
fi
...
kernel_source=$(find "$rootfs/boot" -maxdepth 1 -type f -name 'vmlinuz-*armmp*' | sort | tail -n 1)
initrd_source=$(find "$rootfs/boot" -maxdepth 1 -type f -name 'initrd.img-*armmp*' | sort | tail -n 1)
dtb_source=$(find "$rootfs" -type f -name 'vexpress-v2p-ca9.dtb' | sort | head -n 1)

[ -n "$kernel_source" ] && [ -n "$initrd_source" ] && [ -n "$dtb_source" ] || {
    printf '%s\n' 'FAIL: Debian armhf rootfs did not provide the ARMMP kernel/initrd/vexpress-a9 DTB tuple' >&2
    exit 1
}
...
printf '%s\n' \
    'PASS: native ARMv7/Thumb-2 ELF executed inside a full-system Debian armhf guest' \
