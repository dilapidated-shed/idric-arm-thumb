...
LOW_PRECISION_OBSERVER := build/exec/low-precision-observer
...
.PHONY: check-compiler check driver print-ascii print-ascii-test examples inspect \
	...
	determinism-test numerical-test branching-spec-test low-precision-observe e3m2-observe test verify clean
...
reject-invalid-int: $(DRIVER) tests/source/InvalidInt.idric
	@rm -f build/exec/invalid_int.arm-thumb.S
	@IDRIS2_PATH="$(CURDIR)/build/ttc:$${IDRIS2_PATH}" \
...
reject-too-many-args: $(DRIVER) tests/source/TooManyArgs.idric
	@rm -f build/exec/too_many_args.arm-thumb.S
	@IDRIS2_PATH="$(CURDIR)/build/ttc:$${IDRIS2_PATH}" \
		./$(DRIVER) --cg arm-thumb --source-dir tests/source \
...
reject-invalid-result: $(DRIVER) tests/source/InvalidResult.idric
	@rm -f build/exec/invalid_result.arm-thumb.S
	@IDRIS2_PATH="$(CURDIR)/build/ttc:$${IDRIS2_PATH}" \
		./$(DRIVER) --cg arm-thumb --source-dir tests/source \
...
$(LOW_PRECISION_OBSERVER): $(SCALARS_ASSEMBLY) tests/arm/low_precision_observer.S
	$(ARM_CLANG) --target=$(ARM_EXEC_TARGET) -fuse-ld=lld -nostdlib -static \
		-march=armv7-a -mthumb -mfpu=vfpv3-d16 -mfloat-abi=softfp \
		-Wl,-e,_start -Wl,--no-dynamic-linker \
		$(SCALARS_ASSEMBLY) tests/arm/low_precision_observer.S -o $(LOW_PRECISION_OBSERVER)

low-precision-observe: $(LOW_PRECISION_OBSERVER)
	python3 tests/arm/observe_low_precision.py "$(QEMU_ARM)" "$(LOW_PRECISION_OBSERVER)"

e3m2-observe: $(LOW_PRECISION_OBSERVER)
	python3 tests/arm/observe_low_precision.py "$(QEMU_ARM)" "$(LOW_PRECISION_OBSERVER)" --format E3M2
...
numerical-test: scalar-semantic low-precision-observe source-test lowering-test assembly-test semantic-test determinism-test

# The boring green gate: one real Idriç source program, one Thumb executable,
# exact one-byte observable behavior.
test: print-ascii-test numerical-test branching-spec-test
